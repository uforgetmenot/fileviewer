    ...
    $.fn.pptxToHtml = function (options) {
        ...
        function genShape(node, pNode, slideLayoutSpNode, slideMasterSpNode, id, name, idx, type, order, warpObj, isUserDrawnBg, sType, source) {
            ...
            if (shapType !== undefined || custShapType !== undefined /*&& slideXfrmNode !== undefined*/) {
                var xfrmSourceNode = slideXfrmNode || slideLayoutXfrmNode || slideMasterXfrmNode;
                var off = getTextByPathList(xfrmSourceNode, ["a:off", "attrs"]);
                var x = (off !== undefined && off["x"] !== undefined) ? parseInt(off["x"]) * slideFactor : 0;
                var y = (off !== undefined && off["y"] !== undefined) ? parseInt(off["y"]) * slideFactor : 0;
                x = isNaN(x) ? 0 : x;
                y = isNaN(y) ? 0 : y;

                var ext = getTextByPathList(xfrmSourceNode, ["a:ext", "attrs"]);
                var w = (ext !== undefined && ext["cx"] !== undefined) ? parseInt(ext["cx"]) * slideFactor : 0;
                var h = (ext !== undefined && ext["cy"] !== undefined) ? parseInt(ext["cy"]) * slideFactor : 0;
                w = isNaN(w) ? 0 : w;
                h = isNaN(h) ? 0 : h;
            ...
            if (shapType !== undefined && custShapType === undefined) {
                ...
                var rawPathNodes = getTextByPathList(pathLstNode, ["a:path"]);
                var pathNodes = [];
                if (rawPathNodes !== undefined) {
                    pathNodes = Array.isArray(rawPathNodes) ? rawPathNodes : [rawPathNodes];
                }
                if (pathNodes.length === 0) {
                    console.warn("Custom geometry path list is empty.");
                }

                pathNodes.forEach(function (pathNode) {
                    var pathAttrs = pathNode["attrs"] || {};
                    var maxX = parseInt(pathAttrs["w"], 10);
                    var maxY = parseInt(pathAttrs["h"], 10);
                    ...
                    var moveToNode = getTextByPathList(pathNode, ["a:moveTo"]);
                    if (moveToNode === undefined) {
                        return;
                    }

                    var lnToNodes = pathNode["a:lnTo"]; //total a:pt : 1
                    var cubicBezToNodes = pathNode["a:cubicBezTo"]; //total a:pt : 3
                    var arcToNodes = pathNode["a:arcTo"]; //total a:pt : 0?1? ; attrs: ~4 ()
                    var closeNode = getTextByPathList(pathNode, ["a:close"]); //total a:pt : 0
